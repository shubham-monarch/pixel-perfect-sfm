...
int GeometricBundleOptimizer::AddResiduals<>(
  ...
  if (constant_pose) {
    ...
    ceres::CostFunction* cost_function = CreateGeometricConstantPoseCostFunctor(
        static_cast<int>(camera.model_id), image.CamFromWorld().rotation.coeffs(), image.CamFromWorld().translation, point2D.xy);
    block_id = problem_->AddResidualBlock(cost_function, loss_function, xyz,
                                          camera_params_data);
  } else {
    ceres::CostFunction* cost_function =
        CreateGeometricCostFunctor(static_cast<int>(camera.model_id), point2D.xy);
