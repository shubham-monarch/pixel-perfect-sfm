...
ReferenceExtractor::GetVisibleObservations(
  ...
  for (auto& track_el : point3D.track.Elements()) {
    ...
    WorldToPixel(camera, image.CamFromWorld().rotation.coeffs(), image.CamFromWorld().translation, point3D.xyz,
                 projected.data());
...
OffsetMatrix3d<N_NODES> NodeOffsets3D(
  ...
  const auto& Tmat = image.CamFromWorld();

  //Eigen::Vector2d projected = colmap::ProjectPointToImage(xyz, Tmat, camera);
  Eigen::Vector2d projected = camera.ImgFromCam((Tmat * xyz).hnormalized());
